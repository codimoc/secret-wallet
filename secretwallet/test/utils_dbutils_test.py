...
def test_update_secret_login():
    ...
    try:        
        ...
        assert ns+1 == du.count_secrets()
        assert du.has_secret(domain, access)
        old_ts = du.get_secret(domain, access, m_pwd, salt=key)['timestamp']
...
def test_update_secret_info_change_value():
    ...
    try:        
        ...
        du.update_secret(domain, access, None, None, info_key, info_val, m_pwd, salt=key)
...
def test_update_secret_info_change_password_and_a_value():
    ...
    try:        
        ...
        du.update_secret(domain, access, None, secret_pwd2, info_key, info_val, m_pwd, salt=key)
